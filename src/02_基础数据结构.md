...
普通的**队列**只允许一端入、另一端出；**双端队列（Deque, Double-ended Queue）**则允许在**两端**都能插入和删除。

> 💡 **类比：双端队列如同"双向月台"** 普通队列像单方向的传送带，只能从一头放、另一头取；双端队列则像一排两端都能上下车的站台，乘客（元素）既可以从左头上、也可以从右头上，下车同样两端皆可。这让它在"窗口滑动""从中段维护最值"等场景格外灵活。

| 操作 | 说明 | 复杂度 |
|------|------|:------:|
| `append(x)` / `appendleft(x)` | 右端 / 左端插入 | $O(1)$ |
| `pop()` / `popleft()` | 右端 / 左端删除 | $O(1)$ |
| `queue[0]` / `queue[-1]` | 查看两端元素 | $O(1)$ |
| `rotate(k)` | 整体旋转 k 步（循环移位） | $O(k)$ |

**典型应用**：
- **滑动窗口最值**：配合单调队列思想，用双端队列维护窗口内候选下标（详见 2.9 单调栈/单调队列）。
- **回文判断 / 双向扫描**：两端同时弹出比大小，天然契合双端队列。
- **两栈实现 / 双端栈**：一端做栈、另一端做队列，合并出更丰富的缓冲结构。
- **Replay 缓冲 / 撤销重做**：浏览器前进后退、编辑器 undo/redo 均可用双端队列实现。

Python 的 `collections.deque` 底层用**双向链表**实现，因此两端尾部操作都是 $O(1)$，且弹出后内存自动回收、无数组搬移开销，特别适合需要频繁首尾操作的场景。
...
**集合（Set）**是一种**不含重复元素**的抽象数据结构，核心价值是 $O(1)$ 的"存在性判断"与"去重"。

> 💡 **类比：集合如同"记人名的登记簿"** 一家俱乐部的登记簿记录了所有已入会成员的名字。新成员报到时，工作人员只需看一眼登记簿就能 O(1) 判断"这个人是不是已经入过会"——不用像名单列表那样从头数到尾。集合就是这样一个去重 + 存在性查询的结构。

**两种实现**：
| 实现 | 求平均 | 特点 |
|------|:------:|------|
| **哈希集合（`set`）** | 增删查 $O(1)$ 平均 | Python 内置 `set`，基于哈希表 |
| **有序集合（`sortedcontainers.SortedSet`）** | 增删查 $O(\log n)$ | 红黑树/跳表实现，可有序遍历、取最值 |
...
**复杂度**：哈希集合增删查均摊 $O(1)$、空间 $O(n)$；有序集合增删查 $O(\log n)$。

**典型应用**：去重、存在性判断（判重）、集合运算（并/交/差）、配合哈希表解决"两数之和""环检测"等问题。二叉搜索树（见第 4 章）也可作为有序集合的底层实现。
...
> 🎬 **可视化演示**：单调递增栈求"右边第一个更小元素"（`[4,2,5,3]`，出栈时确定答案）

<img src="images/单调栈.gif" alt="单调栈动画" style="display: block; margin: 20px auto; max-width: 90%;">
