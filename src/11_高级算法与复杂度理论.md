...
def sat_to_vertex_cover(clauses):
    ...
    💡 核心思想：
    - 每个变量构造一条边 (x, ¬x)（变量 gadgets，至少选 1 个端点）
    - 每个子句构造一个三角形（子句 gadgets，至少选 2 个顶点）
    - 子句三角形的每个顶点连向对应文字所在的变量顶点
    - 预算 k = 变量数 n + 2 × 子句数 m

    参数 clauses：子句列表，每个子句是 3 个文字的列表，
    文字用非零整数表示：正数 i 表示 x_i，负数 -i 表示 ¬x_i。
    例如 [[1, -2, 3], [-1, 2, 2]] 表示 (x1∨¬x2∨x3) ∧ (¬x1∨x2∨x2)

    返回：(vertices, edges, k)
    3-SAT 可满足 ⟺ 构造出的图存在大小 ≤ k 的顶点覆盖
    """
    vertices = set()
    edges = []
    ...
    # 文字 -> 变量顶点：正文字连 x_i，负文字连 ¬x_i
    def literal_vertex(lit):
        return ("x", abs(lit)) if lit > 0 else ("¬x", abs(lit))

    # 1) 变量 gadget：对每个变量 x_i，顶点 {x_i, ¬x_i} 之间连一条边
    #    覆盖这条边至少要从两端中选 1 个（对应给变量定一个真值）
    for i in variables:
        pos, neg = ("x", i), ("¬x", i)
        vertices.add(pos)
        vertices.add(neg)
        edges.append((pos, neg))

    # 2) 子句 gadget：每个子句 3 个顶点两两连边成三角形
    #    覆盖三角形的 3 条边至少要选 2 个顶点，
    #    没被选中的那个顶点必须由"子句-变量边"另一端的变量顶点来覆盖
    for j, clause in enumerate(clauses):
        tri = [("c", j, t) for t in range(3)]
        vertices.update(tri)
        ...
        for t, lit in enumerate(clause):
            edges.append((tri[t], literal_vertex(lit)))

    k = len(variables) + 2 * len(clauses)
    return vertices, edges, k


# 示例：(x1 ∨ x2 ∨ x3) ∧ (¬x1 ∨ ¬x2 ∨ ¬x3) 可满足
# vertices, edges, k = sat_to_vertex_cover([[1, 2, 3], [-1, -2, -3]])
# 共 12 个顶点、15 条边，k = 3 + 2×2 = 7：
# 每个变量边选"取值为真"的一端（3 个），
# 每个三角形避开被满足文字对应的顶点、选另外 2 个（共 4 个），
# 恰好构成大小为 7 的顶点覆盖；反之从覆盖也可还原出满足赋值。
```
...
> 💡 **类比：近似比如同"快递承诺的误差范围"** 精确最优（OPT）好比快递的"理想最短路线"，近似算法好比"保证在理想路线长度的 ρ 倍以内送达"。近似比 ρ 越接近 1，效果越好；但换来的是能用多项式时间跑完。这样即使不能保证绝对最优，也能给你一个"误差有界"的可靠解。
...
> 💡 **类比：送快递的最优路线** 快递小哥要走过每个收货点再回仓库。Christofides 算法不追求理论最优（那是 NP-Hard 的 TSP），而是用一个巧妙的拼装法：先连一棵"最小生成树"（连接所有点，保证都能到达），再从树上奇书度点凑成"完美匹配"，把两者合成一个走一圈就能经过所有点的欧拉回路，最后去掉重复点。结果是——**最多比最优路线长 1.5 倍**，而且能多项式时间算完。
...
> 💡 **类比：PTAS 如同"可调精度的放大镜"** 普通近似算法只有一个固定精度（如 2 倍或 1.5 倍）。PTAS 让你**任意设定精度 ε**（想多准就有多准），代价是 ε 越小、运行时间越慢。它就像一个"旋钮可调"的镜头：你转得越细，看得越清楚，但花的时间越多。
...
> 💡 **类比：两类"带随机"的算法** **拉斯维加斯**像"不交卷不罢休"的考生——答案一定正确，只是做题时间有快有慢（期望很快）；**蒙特卡洛**像"按概率给分"的考生——时间固定，但小概率会答错（错误率可人为压低）。做随机化算法前先想清：你是要"保证对，赌时间"，还是"保证时间，赌正确"。
...
> 💡 **类比：蓄水池抽样的"边看边抽"** 数据像一条不断流淌的河流（流式数据），不知道总长、无法一次性全存下。蓄水池抽样模拟的是一块"只留 k 个位置的蓄水池"：每个新元素流过来时，都以恰当概率决定它**是否替换**池中某个旧元素，从而保证最终抽样**等概率**又**只需 O(1) 内存**。就像在看不到尽头的队伍里公平地"抽 K 个幸运观众"。
...
> 💡 **类比：跳跃表如同"带电梯的楼层"** 普通链表只有一层、查找只能一个个走（O(n)）。跳跃表给链表"架空"多层——高层是快速路，相隔更远地跳着走，底层才是完整数据。查找时从最顶层出发一路向下，像走扶梯再转电梯，把查找降到 O(log n)。它是概率化的平衡树平替，代码比红黑树简单得多。
...
> 💡 **类比：LRU 如同"整理书架"** 书架空间有限，你要判断该扔掉哪本书。LRU 的信条是"**最近没用过的先扔**"——因为很久没碰的书，很可能以后也用不上了。每次访问（看书）都把这本书提到书架最显眼处，要淘汰时就把最角落里"久未翻动"的那本拿走。这样高频访问的数据总能留在书架里。
...
> 💡 **类比：布隆过滤器如同"多层指纹登记处"** 想快速知道"这东西我见过吗"，但又不想存全部数据。布隆过滤器用 k 个哈希给每个元素在 k 个位置"盖戳"；查询时看这 k 个位置是否都有戳。**说"没见过"一定准；说"见过"可能误报**（因为别的元素刚好把戳盖重了）。它用极小的空间换来了极快的判断，代价是"宁可错杀，不可放过"的假阳性。
...
> 💡 **类比：Amdahl 定律如同"流水线瓶颈"** 即使把工作效率提到无穷大，整条产线也快不过**不可并行**的那一段。Amdahl 定律告诉你：如果程序里有 s 比例必须串行，那么再怎么加处理器，最高加速比也被 1/s 卡死。它提醒你"并行"的价值要从攻克串行瓶颈开始，而非盲目堆核。
...
> 💡 **类比：金属退火** 冶金时把金属加热再慢慢降温，原子得以从混乱状态逐渐"落"到低能量稳定位。模拟退火把"温度"引入搜索：高温时大胆乱跳（允许接受较差解，躲避局部最优），随温度降低逐渐收敛、稳定在优质解附近。它用"偶尔允许退步"换来了走出生境更高的全局搜索能力。
...
> 💡 **类比：生物进化** 把候选解看成一群"个体"（染色体），用适应度衡量优劣。每一代做三件事：**选择**（留下适应度高的）、**交叉**（个体之间交换片段生成新后代）、**变异**（随机改动一小段以免陷入僵局），一代代"优胜劣汰"逼近最优解。它特别适合解空间巨大、难以用公式刻画的黑盒优化问题。
