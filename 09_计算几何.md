...
> 🎬 **可视化演示**：向量叉积判断转向（左转为正 `cross > 0`，右转为负 `cross < 0`）

<img src="images/向量叉积.gif" alt="向量叉积动画" style="display: block; margin: 20px auto; max-width: 90%;">
...
### 9.2.3 点在多边形内——转角法（Winding Number / 绕数）

> 💡 **类比：绕院子走一圈，数自己转了几圈**
>
> 想象你站在草地中央，闭上眼睛，面朝正东，然后沿着一条想象的路径绕一圈回来：
> - 如果你**在多边形内**，为了让视线始终盯住边界的每一段，你的身体会**不自觉地整整转一圈**（$360^\circ$，即 $2\pi$）——因为你围着多边形绕了一圈
> - 如果你**在多边形外**，你只是"看"着远处的围墙转来转去，身体的总转角恰好**抵消回零**
>
> 这就像带着指南针走一圈：指南针针尖转过的总角度，就是"绕数"。**非零 → 在里面，零 → 在外面**。

下图示意了转角法的判断过程——从内部点 P 出发，沿各条边扫过一圈，张角之和为 $+360^\circ$：

![转角法（绕数）可视化](images/winding_number.svg)

**转角法（Winding Number）**：从目标点 P 出发，沿多边形边的顺序，累加**每条边在 P 处张开的角度**（相邻两条射线 $PV_i$ 与 $PV_{i+1}$ 的夹角，带正负号）。
- 总和为 $+360^\circ$（$+2\pi$）或 $-360^\circ$（$-2\pi$）→ 点在多边形**内部**
- 总和为 $0$ → 点在多边形**外部**

```
内部点 P：各边张角之和 = ±360°（绕了整整一圈）
外部点 P：各边张角之和 = 0（转过去又转回来，正好抵消）
...
| 对比项 | 射线法（奇偶规则） | 转角法（绕数） |
|-------|------------------|---------------|
| 判定依据 | 射线与边界的交点个数（奇数/偶数） | 各边张角的带符号总和（±2π 或 0） |
| 核心思想 | **奇偶规则**（Even-Odd） | **绕数/方向累计**（Winding number） |
| 计算量 | 仅一个水平交点判断，快 | 需要角度累加，略慢 |
| 自交多边形 | 奇偶规则按"填充与否"判定，语义不同 | 能区分"绕了几圈"，可判环绕重数 |
| 精度 | 用叉积/比较，整数友好 | 依赖 $\arctan2$ 或象限累加，浮点敏感 |
| 适用场景 | 简单多边形，面试首选、实现最简 | 需处理自交/环绕、或要"绕数重数"时 |

> 💡 **类比：奇偶 vs 绕数如同"数围栏" vs "数自己转的圈数"**
>
> - 射线法数的是"视线穿过了几道围栏"——奇数在里面，偶数在外面。
> - 转角法数的是"自己走了几整圈"——走了整整一圈就在里面，走了零圈就在外面。
> - 对普通（不自交）多边形，两者结果一致；对**自交多边形**（像五角星），射线法只回答"是否填充"，而转角法还能告诉你某个区域被"绕了几圈"。

```python
import math

def winding_number(point: Point, polygon: list[Point]) -> float:
    """转角法（绕数）判断点是否在多边形内
    ...
    for i in range(n):
        j = (i + 1) % n
        v1 = subtract(polygon[i], point)   # 射线 P→Vi
        v2 = subtract(polygon[j], point)   # 射线 P→Vj

        # 该边在 P 处张开的角（带符号，范围 (-π, π]）
...
def point_in_polygon_winding(point: Point, polygon: list[Point]) -> bool:
    """用转角法判断点是否在多边形内部：绕数非零 → 内部"""
    return abs(winding_number(point, polygon)) > 1e-9

# 示例
polygon = [Point(0, 0), Point(4, 0), Point(4, 4), Point(0, 4)]
print(f"点(2,2)绕数: {winding_number(Point(2, 2), polygon):.2f}")  # ≈ ±6.28（内部）
print(f"点(5,5)绕数: {winding_number(Point(5, 5), polygon):.2f}")  # ≈ 0（外部）
print(point_in_polygon_winding(Point(2, 2), polygon))  # True
print(point_in_polygon_winding(Point(5, 5), polygon))  # False
```

> 💡 **类比：如何不靠反三角函数算绕数——"数半圈"**
>
> $winding(\theta) = \sum_i \Delta\theta_i \in \{-1, 0, 1\}$，其中 $\Delta\theta_i$ 是每条边在 P 处张角的**归一化方向**（每转 $360^\circ$ 记一圈）。实际实现常用"象限累加法"：对每条边判断其跨越的象限，用叉积符号决定 +1 或 -1，全程只用整数运算，避开 $\arctan2$ 的浮点误差——与极角排序里"用叉积替代 $\arctan$"的思路一脉相承。

**复杂度分析**：单次判断只需遍历所有边，时间复杂度 $O(n)$，空间复杂度 $O(1)$。与射线法同样为 $O(n)$，但转角法因需要角度计算（或象限判断）常数更大；非特殊需求下，射线法是面试首选。

> **适用场景**：需要区分自交多边形不同区域的"环绕层数"、或想用绕数做填充/裁剪时。**面试提示**：讲清"绕数非零即内部"的原理、并与射线法的"奇偶规则"对比，是加分项。

### 9.2.4 凸包——Andrew 单调链算法
...
> 🎬 **可视化演示**：Andrew 单调链算法构建凸包（依次连接位于最外围的顶点）

<img src="images/凸包.gif" alt="凸包动画" style="display: block; margin: 20px auto; max-width: 90%;">
